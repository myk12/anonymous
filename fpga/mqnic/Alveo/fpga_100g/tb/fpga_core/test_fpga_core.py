...
async def run_test_nic(dut):
    ...
    # 1. create testbench instance
    #   1.1 instantiate the DUT
    #   1.2 create a PCEI root complex
    #   1.3 create a UtlraScalePlusPcieDevice and connect it to the root complex
    #   1.4 create a Driver
    #   1.5 create an EthMac for each channel
    #   1.6 create an AxiLiteRam for the configuration management space
    tb = TB(dut, msix_count=2**len(dut.uut.core_inst.core_pcie_inst.irq_index))

    # 2. initialize the testbench, reset the DUT 
    await tb.init()
...
@cocotb.test()
async def run_test_time_sync(dut):
    tb = TB(dut, msix_count=2**len(dut.uut.core_inst.core_pcie_inst.irq_index))
    ...
    # indefinitely pull packets from the qsfp_mac and send them
    # to another qsfp_mac
    while True:
        pkt = await tb.qsfp_mac[0].tx.recv()
        await tb.qsfp_mac[1].rx.send(pkt)
